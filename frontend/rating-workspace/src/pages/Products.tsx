  ...
  return (
    <div className="px-6 py-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Product Lines</h1>
      ...
      {products.length === 0 && (
        <div className="bg-white rounded-lg border border-dashed border-gray-300 p-12 text-center">
          ...
          <p className="text-sm font-semibold text-gray-600">No product lines yet</p>
      ...
      {products.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          {products.map((product) => (
            <div
              key={product.code}
              className="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-md transition-shadow flex flex-col gap-3"
            >
              {/* Code + Status */}
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-xl font-bold text-gray-900 font-mono">{product.code}</p>
                  <p className="text-sm font-medium text-gray-700 mt-0.5">{product.name}</p>
                </div>
                <span
                  className={cn(
