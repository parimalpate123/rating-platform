...
function ConfigKV({ config }: { config: Record<string, unknown> }) {
  ...
  if (entries.length === 0) return <p className="text-xs text-gray-400 italic">No config values</p>
  return (
    <dl className="space-y-1">
      {entries.map(([key, value]) => (
        <div key={key} className="flex items-start gap-2">
          <dt className="text-[11px] font-medium text-gray-500 w-28 flex-shrink-0">{key}</dt>
          <dd className="text-[11px] text-gray-800 font-mono break-all">
...
export function StepDetailPanel({ step, result, onClose }: StepDetailPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div
        ref={panelRef}
        className="relative w-80 h-full bg-white border-l border-gray-200 shadow-2xl flex flex-col overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-start justify-between gap-3 px-4 py-4 border-b border-gray-100 flex-shrink-0">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm font-semibold text-gray-900 mt-1.5 leading-snug">{step.name}</p>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 transition-colors flex-shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-lg border border-gray-100 px-3 py-2.5 space-y-1">
              <p className="text-sm font-semibold text-gray-800">{serviceLabel}</p>
              {endpointLabel && (
                <p className="text-xs font-mono text-gray-500">{endpointLabel}</p>
              )}
              ...
              <div className="bg-gray-50 rounded-lg border border-gray-100 px-3 py-2.5">
                <dl className="space-y-1.5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <dd className="text-xs font-mono text-gray-800">{result.durationMs}ms</dd>
                  </div>
                  {result.startedAt && (
                    <div className="flex items-center justify-between">
                      <dt className="text-[11px] text-gray-500">Started</dt>
                      <dd className="text-[11px] font-mono text-gray-600">
          ...
          {result?.output && (
            <div>
              ...
              <pre className="text-[10px] font-mono bg-gray-50 border border-gray-200 rounded-lg p-3 overflow-auto max-h-48 text-gray-700 leading-relaxed whitespace-pre-wrap">
        ...
        <div className="px-4 py-3 border-t border-gray-100 flex-shrink-0">
          <p className="text-[10px] text-gray-400 font-mono truncate">id: {step.id}</p>
        </div>
      </div>
    </div>
  )
