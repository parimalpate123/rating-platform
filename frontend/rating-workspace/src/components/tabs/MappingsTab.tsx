    ...
    <div className={cn('fixed inset-0 bg-black/50 z-50 flex items-center justify-center', maximized ? 'p-0' : 'p-4')}>
      <div className={cn(
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className="flex items-center gap-4 text-xs">
              <span><span className="font-medium text-green-600">{highCount}</span> <span className="text-gray-500">high conf (&ge;80%)</span></span>
              <span><span className="font-medium text-yellow-600">{suggestions.filter((s) => { const p = confPct(s); return p >= 60 && p < 80 }).length}</span> <span className="text-gray-500">medium (60-79%)</span></span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
          {filtered.length === 0 && (
            <div className="text-center py-12 text-sm text-gray-400">No suggestions match the filter.</div>
          )}
          {filtered.map((s, vi) => {
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-600 font-medium flex-shrink-0">{s.transformationType}</span>
                    </div>
                    {s.reasoning && (
                      <div className="mt-1.5 flex items-start gap-1.5 text-[11px] text-gray-500">
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-xl">
          {mode === 'create' && (
            <>
              ...
              {/* Mirror toggle */}
              <label className="flex items-start gap-2 cursor-pointer mb-3 p-2 rounded-lg hover:bg-gray-100 transition-colors">
                <input
                  type="checkbox"
                  checked={mirror}
                ...
                <div>
                  ...
                  {mirror && reverseName && (
                    <p className="text-[11px] text-gray-500 mt-0.5">
                      Will create <span className="font-medium text-gray-700">"{reverseName}"</span> with {selected.size} field{selected.size !== 1 ? 's' : ''} swapped
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-500">{selected.size} of {suggestions.length} selected</span>
    ...
    <div className={cn('fixed inset-0 bg-black/40 z-50 flex items-center justify-center', maximized ? 'p-0' : 'p-4')}>
      <div className={cn(
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          {/* Method selection */}
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-2">Creation Method</label>
              ...
              {METHODS.map((m) => (
                <button
                  ...
                  className={cn(
                    ...
                    m.disabled ? 'opacity-50 cursor-not-allowed border-gray-200 bg-gray-50' :
          ...
          {method === 'manual' && (
            <>
              ...
              <label className="flex items-start gap-2 cursor-pointer p-2.5 rounded-lg border border-dashed border-gray-200 hover:bg-gray-50 transition-colors">
                <input
                  type="checkbox"
                  checked={mirror}
                  onChange={(e) => setMirror(e.target.checked)}
            ...
            <div className="space-y-3">
              <div>
                ...
                <label className={cn(
                  ...
                  maximized ? 'h-64' : 'h-32',
                  csvFile ? 'border-green-400 bg-green-50' : 'border-gray-300 bg-gray-50 hover:bg-gray-100',
                )}>
                  <Upload className={cn('w-6 h-6 mb-2', csvFile ? 'text-green-600' : 'text-gray-400')} />
                  {csvFile ? (
                    ...
                    <>
                      ...
                      <span className="text-xs text-gray-400 mt-0.5">CSV, TSV files accepted</span>
                    </>
                  )}
                  <input
                    type="file"
        ...
        {/* Footer */}
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50 rounded-b-xl flex-shrink-0">
          <button onClick={onClose} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">Cancel</button>
...
function TransformConfigFields({
  ...
  const inputCls = 'w-full px-2 py-1.5 text-xs border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white'
    ...
    <div className="border-t border-gray-100 pt-2">
      ...
      {open && (
        <div className="mt-2 space-y-2">
          {/* Description */}
          <div>
            <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Description</label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Default Value</label>
            ...
            <div>
              <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Field Identifier</label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Data Type</label>
            ...
            <div>
              <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Field Direction</label>
          ...
          <div className="flex items-center gap-3 py-0.5">
            <label className="flex items-center gap-1.5 cursor-pointer">
              ...
              />
              <span className="text-xs text-gray-600 font-medium">Skip Mapping</span>
            </label>
            {details.skipMapping && (
              <select value={details.skipBehavior} onChange={(e) => set('skipBehavior', e.target.value)} className="px-2 py-1 text-xs border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Sample Input</label>
              <input type="text" value={details.sampleInput} onChange={(e) => set('sampleInput', e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Sample Output</label>
...
function AddFieldRow({ mappingId, onAdded }: { mappingId: string; onAdded: () => void }) {
  ...
  if (!show) {
    return (
      <button onClick={() => setShow(true)} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
  ...
  return (
    <div className="border border-blue-200 bg-blue-50/30 rounded-lg p-3 space-y-2">
      <div className="grid grid-cols-[1fr_1fr_120px_60px] gap-2 items-center">
        <input type="text" value={form.sourcePath} onChange={(e) => setForm((f) => ({ ...f, sourcePath: e.target.value }))} className="px-2 py-1.5 text-xs border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white" placeholder="Source path (e.g. policy.state)" />
        <input type="text" value={form.targetPath} onChange={(e) => setForm((f) => ({ ...f, targetPath: e.target.value }))} className="px-2 py-1.5 text-xs border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white" placeholder="Target path (e.g. State)" />
        <select value={form.transformationType} onChange={(e) => setForm((f) => ({ ...f, transformationType: e.target.value as TransformationType, transformConfig: {} }))} className="px-2 py-1.5 text-xs border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white">
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <button onClick={() => setShow(false)} className="px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">Cancel</button>
      </div>
    </div>
  )
}
...
function FieldRow({ field, mappingId, onChanged }: { field: FieldMapping; mappingId: string; onChanged: () => void }) {
  ...
  if (editing) {
    return (
      <div className="border border-blue-200 bg-blue-50/30 rounded-lg p-3 space-y-2">
        <div className="grid grid-cols-[1fr_1fr_120px_60px] gap-2 items-center">
          <input type="text" value={form.sourcePath} onChange={(e) => setForm((f) => ({ ...f, sourcePath: e.target.value }))} className="px-2 py-1.5 text-xs border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white" />
          <input type="text" value={form.targetPath} onChange={(e) => setForm((f) => ({ ...f, targetPath: e.target.value }))} className="px-2 py-1.5 text-xs border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white" />
          <select value={form.transformationType} onChange={(e) => setForm((f) => ({ ...f, transformationType: e.target.value as TransformationType, transformConfig: {} }))} className="px-2 py-1.5 text-xs border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white">
            {TRANSFORM_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
          </select>
          <label className="flex items-center gap-1 text-xs text-gray-600 cursor-pointer">
  ...
  return (
    <div className={cn('rounded-lg group', isSkipped ? 'opacity-60' : '')}>
      <div className="grid grid-cols-[1fr_1fr_100px_36px_auto] gap-2 items-center py-2 px-3 hover:bg-gray-50 rounded-lg">
        <span className={cn('text-xs font-mono text-gray-700 truncate', isSkipped && 'line-through text-gray-400')}>{field.sourcePath}</span>
        <span className={cn('text-xs font-mono text-gray-700 truncate', isSkipped && 'line-through text-gray-400')}>{field.targetPath}</span>
        <span className={cn('inline-flex items-center justify-center px-2 py-0.5 rounded text-[10px] font-medium', isSkipped ? 'bg-gray-100 text-gray-400' : typeColor)}>{field.transformationType}</span>
        <span className="text-center">{field.isRequired ? <Check className="w-3 h-3 text-green-600 mx-auto" /> : <span className="text-gray-300 text-xs">—</span>}</span>
        <div className="flex items-center gap-1.5 justify-end">
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={startEditing} className="p-1 text-gray-400 hover:text-blue-600"><Pencil className="w-3 h-3" /></button>
            <button onClick={handleDelete} className="p-1 text-gray-400 hover:text-red-600"><Trash2 className="w-3 h-3" /></button>
          </div>
        </div>
      </div>
      ...
      {field.description && (
        <div className="px-3 -mt-1 pb-1">
          <span className="text-[10px] italic text-gray-400">{field.description}</span>
        </div>
      )}
    </div>
  )
          ...
          {editMode ? (
            ...
            <div className="flex-1 flex items-center gap-2 px-4 py-2.5">
              <input
                ...
                autoFocus
                className="flex-1 px-2.5 py-1.5 text-sm border border-blue-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Mapping name..."
              />
              ...
              <button onClick={handleEditCancel} className="px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-100 rounded-lg flex-shrink-0">
                Cancel
              </button>
            </div>
          ) : (
            <button onClick={toggle} className="flex-1 flex items-center gap-3 px-4 py-3 text-left hover:bg-gray-50 transition-colors min-w-0">
              ...
              {/* Number badge */}
              <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center rounded-full bg-gray-100 text-[10px] font-semibold text-gray-500">
                {index}
              </span>
              {/* Name */}
              <span className="text-sm font-medium text-gray-800 flex-1 truncate min-w-0">{mapping.name}</span>
              {/* Direction */}
              ...
              {/* Date */}
              <span className="text-[11px] text-gray-400 flex-shrink-0 hidden md:block">
                {fmtDate(mapping.createdAt)}
              </span>
              {/* User */}
              <span
                className="flex-shrink-0 px-2 py-0.5 rounded-full bg-gray-100 text-[10px] text-gray-500 font-medium hidden md:block"
          ...
          {!editMode && (
            <>
              ...
              <button
                ...
                className="p-1.5 text-gray-300 hover:text-blue-500 rounded transition-colors flex-shrink-0"
              >
          ...
          <div className="border-t border-gray-100 px-4 py-3 space-y-1">
            {loading && (
              <div className="flex items-center gap-2 py-3">
                ...
                <span className="text-xs text-gray-400">Loading fields...</span>
              </div>
            )}
            ...
            {!loading && fields.length > 0 && (
              <>
                <div className="grid grid-cols-[1fr_1fr_100px_36px_auto] gap-2 px-3 pb-1">
                  <span className="text-[10px] font-semibold text-gray-400 uppercase">Source</span>
                  <span className="text-[10px] font-semibold text-gray-400 uppercase">Target</span>
                  <span className="text-[10px] font-semibold text-gray-400 uppercase text-center">Type</span>
                  <span className="text-[10px] font-semibold text-gray-400 uppercase text-center">Req</span>
                  <span />
                </div>
                {fields.map((f) => (
            ...
            {!loading && fields.length === 0 && loaded && (
              <p className="text-xs text-gray-400 py-2">No field mappings configured yet.</p>
            )}
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-600 border border-blue-200 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors"
          >
        ...
        {mappings.length === 0 ? (
          <div className="bg-white rounded-lg border border-dashed border-gray-300 p-12 text-center">
            ...
            <p className="text-sm font-medium text-gray-600">No mappings yet</p>
